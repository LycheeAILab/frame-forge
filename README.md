...
同一只金毛，同一个车窗，五种表达。以下预览均由 Frame Forge 使用内置生图工具制作。
...
| 5 · 纸胶带拼贴 |
| :---: |
| <img src="assets/previews/masking-tape.png" width="300" alt="纸胶带拼贴示例"> |
| 上方保留照片，下方以约 10–18 块纸胶带重构主体；暖白纸张、大留白、柔和配色 |
...
```text
用 $frame-forge 展示五种风格的示例图，我选好后再生成。
```
...
- **风格选择**：直接匹配名称或编号，未指定则展示上方五张随包固定示例（支持时以两列展示），客户回复编号后再生成。选择页不额外生图。
- **验证状态**：五种风格均已完成单张示例生成与视觉检查；照片保留效果以实际结果为准，不承诺像素级一致。
